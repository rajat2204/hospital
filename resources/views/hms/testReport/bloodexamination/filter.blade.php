...
<div class="app-content  my-3 my-md-5">
    <div class="side-app">
        <div class="page-header">
            <h5 class="page-title"> BLOOD EXAMINATION PATIENT LIST</h5>
            <ol class="breadcrumb">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="col-md-12">
                            ...
                            <a href="{{ route('blood-examination.index') }}">
                                <p class=" btn btn-success btn-xs">
                                    <i class="fa fa-plus"></i>  BLOOD EXAMINATION 
                                </p>
                            </a>
                        </span>
                        <span class="pull-right" style="margin-right: 5px;">
                              <a href="javascript:;" class="btn btn-info btn-xs" onclick="printDiv('BLOOD EXAMINATION - PATIENTS LIST','BloodExamTable')"> <i class="fa fa-print"></i> PRINT </a>
                        </span>
                        </div>
                    </div>
                    <div class="card-body">
                       <div class="table-responsive">
                            <table id="BloodExamTable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        oTable = $('#BloodExamTable').DataTable({
            "lengthMenu": [[100, 250, 500, 1000], [100, 250, 500, 1000]],
            processing: true,
            serverSide: true,
            bDestroy: true,
            ajax: "{{ route('blood.filterData') }}",
            dom: 'Blfrtip',
          buttons: [
    ...
    var obj = {
       ...
       bloodReportDelete:function(data){
            $.ajax({
                url: data.action,
                type: 'DELETE',
                data: data,
                success:function(res){
                    if(res.status==true){
                         $('#BloodExamTable').DataTable().ajax.reload();
                         swal({
                            title:"Delete",
       ...
       jQuery(document).ready(function() {
        	 jQuery(document).on('click','.bloodexamView',function() {
    		    var id = $(this).data('id');
    		    var _token = $("input[name='_token']").val();
    		    var action = '{{ route('blood.reportView') }}';
    		    var data = {id:id,_token:_token,target:'#bloodReportView',modal:'#id-bloodReportView',action:action};
    		    obj.bloodReportView(data);
    		});
        });

        $(document).on('click','.bloodexamDelete',function() {
            ...
            if(!confirm('Are you sure want to Delete?')) {    
                    ...
                    var id = $(this).data('id');
                    var _token=$("input[name='_token']").val();
                    var action='{{  url('hms/test-report/blood-examination') }}'+'/'+id;
                    var data={_token:_token,action:action};
                    obj.bloodReportDelete(data);
                }
        });
</script>
@endpush
